(function ($) {
    ...
    function OrgChart($container, opts) {
        ...
        this.draw = function () {
            ...
            $container.find('.org-edit-button').click(function () {
                if (self.opts.onClickNode !== null) {
                    self.opts.onClickNode(nodes[$(this).parent().attr('node-id')]);
                }
            });

            if (opts.allowEdit && opts.allowNameEdit) {
                $container.find('.node .node-text').click(function (e) {
        ...
        this.startEdit = function (id) {
            ...
            $container.find('div[node-id=' + id + '] .node-text').replaceWith(inputElement);
            var commitChange = function () {
                var h2Element = $('<div class="node-text" style="font-size:' + opts.nameFontSize + ';">' + nodes[id].data.name + '</div>');
                if (opts.allowEdit) {
        ...
        this.deleteNode = function (id) {
            ...
            if (nodes[nodes[id].data.parent] !== undefined) {
    ...
    function Node(data) {
        ...
        this.formatNode = function (opts) {
            ...
            if (typeof data.name !== 'undefined') {
                nameString = '<div class="node-text" style="font-size:' + opts.nameFontSize + ';">' + self.data.name + '</div>';
            }
            if (typeof data.description !== 'undefined') {
                descString = '<p>' + self.data.description + '</p>';
            }
            if (opts.showControls) {
                var buttonsHtml = "<div class='org-add-button'>" + opts.newNodeText + "</div><div class='org-edit-button'></div><div class='org-del-button'></div>";
            }
            else {
                buttonsHtml = '';
            }
